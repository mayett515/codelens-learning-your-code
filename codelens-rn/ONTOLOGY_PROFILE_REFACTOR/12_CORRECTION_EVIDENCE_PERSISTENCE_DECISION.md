...
- `NEXT_LLM_CONTEXT.md`: This decision is now implemented as storage-only evidence. The adaptive suggestion policy is locked in doc 18, patch/merge proposal storage is locked and implemented as storage-only v1 in doc 19, trust setting storage is implemented in doc 23, and the first branch-local proposal review/apply seam is locked in doc 24. Remaining work is implementation of branch-local apply helpers/UI, event/audit projections, context assembly, base-profile versioning, agent/subagent execution ontology, and self-building-app framework.
