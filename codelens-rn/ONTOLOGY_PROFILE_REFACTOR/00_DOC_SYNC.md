...
- `43_SECOND_BASE_PROFILE_FORKABILITY_DEMO_DECISION.md` locks the first second-base-profile proof as a minimal photography forkability demo. It should prove the existing registry, profile-definition persistence, project selection, branch creation, checker/proposal review, target-switch, and base apply seams work against a non-coding base profile. It must treat required production-code changes as coupling findings, not as a license for profile-gallery UX, cross-base behavior, new operation vocabulary, maturity lifecycle, or source-sync work.
