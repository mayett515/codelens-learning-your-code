...
- [43_SECOND_BASE_PROFILE_FORKABILITY_DEMO_DECISION.md](43_SECOND_BASE_PROFILE_FORKABILITY_DEMO_DECISION.md) - locked decision + implementation: the first forkability proof adds a minimal photography base profile and tests that exercise registry, persisted profile definitions, backup row mapping, project selection, branch composition, checker/proposal review, target switching, and base apply against a non-coding profile. The follow-up implements profile-scoped learning concept-list/retrieval filters for overlapping type ids such as `composition`. The deliverable is the profile proof plus discovered coupling fixes/findings, not profile-gallery UX, cross-base behavior, new operation vocabulary, maturity lifecycle, source-sync, auto-apply, or a shipping decision for photography.
