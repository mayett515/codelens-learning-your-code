...
- [28_CONTEXT_ASSEMBLY_DECISION.md](28_CONTEXT_ASSEMBLY_DECISION.md) - locked docs-only decision: Kordex context assembly uses one shared typed `ContextPack` contract with scoped ontology refs, structured policy, evidence/proposal snapshots, deterministic caps, and mandatory `compositionStamp` including `branchOrder`. The first implementation slice is pure builder + invariant validator + canonical deterministic JSON serializer only; no prompt renderer, DB, LLM, UI, retrieval/ranking, graph engine, or token optimization.
