...
- [39_EDIT_THEN_APPLY_DECISION.md](39_EDIT_THEN_APPLY_DECISION.md) - locked decision + first implementation: richer missing-concept edit/apply is a review-time draft flow over pending proposals. The first slice adds editable missing-concept meaning and proposal provenance that records the original suggestion plus edited fields. Branch targets still apply through doc 24, base/core targets through doc 38. No direct Conceptualize apply, target switching UI, automatic missing-concept apply, checker runtime, stale refresh, old-card backfill, or auto-apply is added.
