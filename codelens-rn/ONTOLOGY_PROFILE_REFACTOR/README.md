...
- [24_BRANCH_LOCAL_PROPOSAL_APPLY_DECISION.md](24_BRANCH_LOCAL_PROPOSAL_APPLY_DECISION.md) - locked decision + first helper/service/minimal UI implementation: first proposal apply flow is explicit, branch-local, revalidated, and atomic. `branchLocalProposalApply.ts` compiles pending branch proposals into typed operations and merges patches into branch overlay values, while `data/branchLocalProposalApplyService.ts` loads proposal/branch rows and commits branch update plus accepted proposal in one transaction. The first review UI adds a Learning Hub entry and queue/detail modal for Apply, Reject, Postpone, and Ask why / why not without edit support. Ask why records audit history but does not mutate proposal status, branch/base state, evidence, or ontology. No base/core mutation, upward merge, old-card backfill, edit-then-apply, or auto-apply is part of the first apply seam.
- [25_PROPOSAL_EVENT_AUDIT_STORAGE_DECISION.md](25_PROPOSAL_EVENT_AUDIT_STORAGE_DECISION.md) - locked decision + implementation: proposal review/apply/lifecycle decisions are append-only event facts in `profile_proposal_events`. Apply/Reject/Postpone/Ask-why/Superseded now write audit events inside the same guarded transaction as their proposal/branch/lifecycle state changes, except Ask why which intentionally preserves pending status and branch/base state. Superseded and Ask-why events are audit history, not user-fit preference; no checker runtime, auto-apply engine, undo execution, or base/core mutation is added.
...
- [39_EDIT_THEN_APPLY_DECISION.md](39_EDIT_THEN_APPLY_DECISION.md) - locked decision + implementation slices: richer missing-concept edit/apply is a review-time draft flow over pending proposals. Implemented slices add editable missing-concept meaning, proposal provenance, target readout, proposal-review handoff, superseding lifecycle, event-history readout, a same-target edited replacement service/hook, a first visible proposal editor for supported new-node proposals, target-layer switching scope, a pure target-switch eligibility/blast-radius helper, a data-layer target-switch replacement/supersede service, and a review-surface move-to-core action. Branch targets still apply through doc 24, base/core targets through doc 38. First target switching is scoped to explicit branch-local additive proposal -> base/core replacement-plus-supersede with fresh base version snapshot. No direct Conceptualize apply, automatic missing-concept apply, checker runtime, old-card backfill, auto-apply, or target switching outside this first branch-to-base additive scope is added.
- [40_PROPOSAL_FRESHNESS_AND_STALE_REFRESH_DECISION.md](40_PROPOSAL_FRESHNESS_AND_STALE_REFRESH_DECISION.md) - locked decision + implementation: proposal freshness is separate from proposal review status. Branch target revision snapshots, the pure freshness helper, proposal review freshness readout, and explicit refresh/rebase replacement creation are implemented. Stale/conflicted/obsolete are derived review conditions, not proposal statuses. Refresh creates a replacement pending proposal and supersedes the old one; it does not edit history, auto-apply, widen targets, or mutate profiles silently.
- [41_CHECKER_RUNTIME_FIRST_SLICE_DECISION.md](41_CHECKER_RUNTIME_FIRST_SLICE_DECISION.md) - locked decision + first implementation slices: consolidates the future checker vision from docs 03/07/21/37/Fable and locks the first runtime slice as manual-on-demand, branch-local, proposal-only, additive ontology-node/item-type proposals only. The pure checker prompt/output contract, deterministic proposal mapper, manual runtime service behind the ontology data boundary, user-facing `Run checker now` trigger/readout, concrete model adapter seam, correction-pattern `patternFrequency` aggregation, and `isItemType` parent hints are implemented. Relationship, boundary, split/merge/rename/deprecate/move, maturity, base/core checker targeting, background runs, checker-run tables, and auto-apply are deferred.
- [42_BRANCH_PROFILE_SELECTION_UI_DECISION.md](42_BRANCH_PROFILE_SELECTION_UI_DECISION.md) - locked decision + first implementation slices: the first branch/profile selection UI is a minimal project-scoped selector over existing profile selection and branch persistence seams. The pure selection-draft helper groups branch options, validates selected base/branch ids, preserves selected order, and computes explicit checker-target state. Focused data hooks load/save project selection, load branches, and create empty branch rows. The compact selection panel is wired into proposal review, can create/select/reorder empty branches, and gives the checker an explicit branch target when project context is available. Project-owned entry points now pass `projectId` explicitly into the Learning Hub/proposal review path. No new schema, global active selection, composed-profile persistence, target switching, merge/fork UX, multi-base composition, base mutation, or auto-apply is added.
- [43_SECOND_BASE_PROFILE_FORKABILITY_DEMO_DECISION.md](43_SECOND_BASE_PROFILE_FORKABILITY_DEMO_DECISION.md) - locked decision + implementation: the first forkability proof adds a minimal photography base profile and tests that exercise registry, persisted profile definitions, backup row mapping, project selection, branch composition, checker/proposal review, target switching, and base apply against a non-coding profile. Follow-ups implement profile-scoped learning capture/save/precheck/list/retrieval plus Learning Hub/chat retrieval, graph, review, and promotion consumers for overlapping type ids such as `composition`. The deliverable is the profile proof plus discovered coupling fixes/findings, not profile-gallery UX, cross-base behavior, new operation vocabulary, maturity lifecycle, source-sync, auto-apply, or a shipping decision for photography.
...
- [FABLE_STRATEGIC_REVIEW_2026-06-09/](FABLE_STRATEGIC_REVIEW_2026-06-09/00-system-index.md) - historical Claude Fable strategic architecture review pack. Durable bans and routing guidance are folded into active docs; use this only as review guidance. It does not override numbered decision docs.
...
- [FUTURE_CONCEPTS/GAME_ENGINE/](FUTURE_CONCEPTS/GAME_ENGINE/README.md) - non-authoritative game-engine inspiration and the standalone technical Kordex explainer. These preserve future application ideas without reopening numbered decisions.
...
- [NEXT_LLM_CONTEXT.md](NEXT_LLM_CONTEXT.md) - canonical active handoff for the next worker/reviewer while this refactor is active.
- [implementation_handoff.md](implementation_handoff.md) - historical implementation log; use `NEXT_LLM_CONTEXT.md` first.
