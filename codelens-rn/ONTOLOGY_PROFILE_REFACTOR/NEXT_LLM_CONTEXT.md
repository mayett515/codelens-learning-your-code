...
- Doc 39 is locked and the first missing-concept draft/edit slices are implemented in the save-learning store, Conceptualize correction controls, `conceptualizeProfileContext.ts`, `saveConceptualizedCapture.ts`, and the ontology proposal edit service/hook.
...
```text
1. Continue doc 39 edit-flow UI only after the proposal replacement path is reviewed.
```
...
```text
Proposal edit-flow editor UI:
  - proposal freshness/readout/refresh creation now exists
  - doc 39 now has the data/hook seam for edited replacements
  - doc 39 still lacks the visible editor UI that feeds that seam
  - the editor must reuse proposal-first replacement + superseding history
  - it must not apply automatically, silently widen targets, rewrite old cards, or mutate base/branch state directly
```

Likely implementation sequence after audit:

1. Review the refresh creation slice.
2. Then implement the doc 39 replacement-proposal editor surface if still coherent.
