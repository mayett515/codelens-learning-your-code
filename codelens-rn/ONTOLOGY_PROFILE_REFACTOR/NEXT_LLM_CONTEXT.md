...
```text
Target switching decision or manual checker gate:
  - proposal lifecycle, freshness, refresh, and same-target edit UI now exist
  - doc 39 still defers target-layer switching and direct Apply from edit
  - Fable recommends manual checker after lifecycle is ready
  - decide before implementation whether the next product value is target switching, manual checker, or branch/profile selection UI
```
