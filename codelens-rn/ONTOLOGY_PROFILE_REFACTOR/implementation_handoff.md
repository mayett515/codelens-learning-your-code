...
29. The remaining open work is:
    - runtime activation helper implementation - interface-based project/context resolver before UI/service wiring
...
38. The runtime activation wiring decision is locked in doc 16. The next code slice should add an interface-based project/context resolver, likely in `src/features/ontology/runtimeProfileActivation.ts`, that uses caller-supplied selection/profile/branch stores and delegates to existing pure helpers. It should not import `db/client`, services, UI, MCP/adapters, agent runtime, app-builder runtime, or DSL runtime.
