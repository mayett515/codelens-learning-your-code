...
29. The remaining open work is:
    - Conceptualize first implementation scope - existing-tag correction only, or existing-tag correction plus branch-local new tag/subtag creation
    - trust setting storage - where conservative/suggest-first/adaptive settings and user-fit learning live
...
Latest verification after profile change proposals storage v1:
...
Result: TypeScript clean; targeted migration/codec/backup/guard tests 121/121 passed across 4 files.
Full suite after this slice: 691/691 passed across 70 files. `git diff --check` clean with CRLF warnings only.
