...
- Added `41_CHECKER_RUNTIME_FIRST_SLICE_DECISION.md` to consolidate the scattered future checker vision from docs 03/07/21/37/Fable and lock the first checker runtime slice. The first runtime is manual-on-demand, branch-local, proposal-only, additive ontology-node/item-type proposals only, and it reuses existing ContextPack/user-fit/proposal review/apply/freshness/edit infrastructure. The pure prompt/mapper source slice is implemented: `checkerPromptBuilder.ts` builds and validates checker-only output over a checker `ContextPack`, while `checkerProposalMapper.ts` maps validated `missing_branch_item_type` findings into pinned pending branch-local `ontology_node_patch` proposal values. The mapper contract pins checker proposals to `sourceKind: 'checker'`, `sourceBranchId: null`, `proposalKind: 'ontology_node_patch'`, one concept/node per proposal, checker-minted nodes as `createdBy: 'model'` and `status: 'active'`, branch target snapshots, deterministic risk, cap-after-validation/dedup, and insert-only duplicate handling. The manual runtime service is now implemented in `data/checkerRunService.ts`: it loads bounded facts, assembles a checker `ContextPack`, calls an injected checker model seam outside the write transaction, validates output, maps findings to branch-local proposals, dry-runs candidates through the branch-local compile path, skips conflicts/duplicates with explanation, and inserts only passing pending proposals. The UI/model seam policy is locked before the UI trigger slice. Relationship changes, boundary-rule edits, split/merge/rename/deprecate/move, temporary/provisional relationship/tag lifecycle, maturity transitions, base/core checker targeting, background/event/scheduled runs, checker-run tables, and auto-apply are deferred. No UI trigger, concrete LLM/provider adapter implementation, operation-vocabulary expansion, direct profile mutation, or auto-apply behavior was added.
...
- Added proposal event audit storage: `019-profile-proposal-events` creates `profile_proposal_events`; `ProfileProposalEvent` records append-only Apply/Reject/Postpone/Ask-why decision facts with actor, target, before/after proposal status/timestamps, optional branch before/after timestamps, reason, and details JSON. `data/branchLocalProposalApplyService.ts` now appends an `applied` event in the same transaction as the branch/proposal update, and `data/profileChangeProposalReviewService.ts` appends `rejected` / `postponed` events in the same transaction as proposal review. Ask why now appends `asked_why` with `pending -> pending` status and unchanged proposal/profile state. Migration 023 extends events with `superseded`, and `data/profileChangeProposalLifecycleService.ts` appends `superseded` events when an old pending proposal is linked to an already-created pending replacement. `supersedeProfileChangeProposal()` / `useSupersedeProfileChangeProposal()` expose that transition for edit UI code, and `createEditedProfileChangeProposalReplacement()` / `useEditProfileChangeProposal()` now create same-target edited replacements before superseding the old proposal. Backup/export/import/clear/columnMaps and stage10 guards support the table. Proposal review now displays selected-proposal event history. No checker runtime, auto-apply engine, historical undo execution, base/core mutation, old-card backfill, agent runtime, app-builder runtime, or DSL runtime was added.
- Added the first missing-concept edit/apply implementation slice from doc 39. Correction drafts now carry editable `newTypeMeaning`; `Use suggestion` copies suggested label, meaning, reason, and valid parent into the draft; proposal nodes use edited meaning before suggestion/reason fallback; and proposal reasons preserve the original suggested label/parent/meaning/reason plus edited-field provenance. The store still does not auto-fill suggestions on load. Superseding lifecycle persistence and the same-target edited replacement service now exist behind the ontology data boundary, but the visible UI editor is not wired yet. No direct Conceptualize Apply, target-layer switching UI, old-card backfill, checker runtime, auto-apply, graph/vector retrieval, trust-setting update, agent runtime, app-builder runtime, or DSL runtime was added.
- Added the doc 39 target readout slice. Conceptualize correction controls now show the current proposal target and branch/base blast-radius copy for missing-concept reviews and new-subtype drafts. This is display-only: no target switching controls, direct Conceptualize Apply, auto-apply, or profile mutation was added.
- Added the doc 39 proposal-review handoff slice. Saving a draft that creates a pending profile-change proposal returns that proposal to the modal, saved candidate cards show `Review proposal`, and the button opens the existing proposal review/apply surface with that proposal selected. Apply remains guarded by doc 24/doc 38 services; no one-click direct Conceptualize Apply or hidden mutation was added.
- Added `40_PROPOSAL_FRESHNESS_AND_STALE_REFRESH_DECISION.md` as the stale refresh contract. Freshness is a derived review condition, not proposal status. Branch target revision snapshots, the pure freshness helper, proposal review freshness readout, and explicit refresh/rebase replacement creation are now implemented. Refresh creates a replacement pending proposal and supersedes the old pending proposal; it does not edit old proposal history, auto-apply, silently widen targets, rewrite cards/evidence, or mutate profiles.
...
29. The remaining open work is:
    - user-fit projection over proposal events and near-miss/missing-concept correction evidence
    - DB-backed correction/proposal history facts reader for checker/context use
    - base profile versioning - how accepted operations safely target base profiles
    - missing-concept apply/proposal revalidation beyond the existing manual correction path
...
47. The first minimal proposal review UI is implemented. `ProfileProposalReviewEntry` appears in the Learning Hub only when pending proposals exist, and `ProfileProposalReviewScreen` opens a queue/detail modal for pending `profile_change_proposals`. The detail view shows target, proposal kind, blast-radius risk wording, semantic confidence, user-fit confidence, patch summary, reason, and evidence. Apply is wired through `useApplyProfileChangeProposal`, which resolves the default profile registry above the data service and calls the branch-local apply service. Reject/Postpone are wired through `useReviewProfileChangeProposal` and `data/profileChangeProposalReviewService.ts`, which conditionally updates only pending proposals. Ask why / why not records audit history without changing proposal status or profile data. Model-review hardening added missing-base-profile error mapping, explicit success/error message tone, non-branch Apply disablement, future branch-key invalidation, pending-action list switching, reason/message reset, ontology-node names in patch summaries, and broader presentation tests. No edit support, checker runtime, auto-apply engine, upward merge, base/core mutation, old-card backfill, agent runtime, app-builder runtime, or DSL runtime was added.
48. The proposal event audit storage decision is locked and implemented in doc 25. `profile_proposal_events` stores append-only proposal decision/lifecycle facts. Apply / Reject / Postpone / Ask-why / Superseded insert events inside the same guarded transactions or review action boundaries; if a conditional write conflicts, no event is written. Superseded and Ask-why events are audit history, not user-fit preference. The proposal review surface now displays selected-proposal event history and can save supported new-node proposal edits through the same replacement/superseding path. No checker runtime, auto-apply engine, historical undo execution, base/core mutation, old-card backfill, agent runtime, app-builder runtime, or DSL runtime was added.
49. The context assembly decision is locked and implemented in doc 28. `src/features/ontology/contextAssembly.ts` exposes typed ContextPack structures plus `assembleContextPack`, `validateContextPack`, `assertValidContextPack`, `serializeContextPack`, and `scopedNodeRefKey`. The pack includes bounded advisory `userFit` signals when callers supply them. It is a pure projection boundary: no DB, UI, LLM, retrieval, graph traversal, prompt renderer, checker runtime, apply/mutation, or runtime behavior change.
50. The context selector decision is locked and the first pure implementation slice is implemented in doc 29. `contextSelector.ts` exposes `ContextSelector`, `ContextSelection`, trace types, `createConceptualizeContextSelector`, and `selectConceptualizeContext`. The selector works over caller-supplied ordered candidates, uses pinned/elastic buckets, preserves same-label ambiguity and cross-scope evidence, keeps evidence tied only to elastic ontology context capped, traces cap omissions, and clones inputs rather than mutating caller objects. It is exported from the ontology root barrel. No DB, UI, LLM, retrieval, graph traversal, prompt rendering, checker runtime, apply/mutation, base/core versioning, agent runtime, app-builder runtime, DSL runtime, or runtime behavior wiring was added.
51. The Conceptualize ContextPack shadow wiring decision is locked and implemented in doc 30. `conceptualizeContextPack.ts` builds and validates a ContextPack from real prepared Conceptualize candidates and enriched profile context. This started as a shadow-only path and is now reused by the classification-only Extractor Flip. Doc 37 now feeds bounded advisory user-fit signals from the resolved profile context into this pack, scoped to the exact active selection; suggestions, proposals, ontology/profile state, DB readers, vector retrieval, graph traversal, and learned-score persistence remain out of the context builder.
52. The Conceptualize prompt-builder decision is locked and implemented in doc 31. `conceptualizePromptBuilder.ts` consumes validated ContextPacks and renders a stable instruction shell, compact Kordex context payload JSON including advisory `userFit` history, strict output schema, and output validator. Unknown refs are rejected against the original pack instead of being guessed from labels.
53. The Conceptualize singular-output/diagnostics decision is locked and implemented in doc 32. Public `additionalNodeRefs` are removed. Conceptualize now exposes one public classification result while `diagnostics.candidateRefs` remains internal-only, dynamic policy-driven calibration data. The diagnostic budget is derived from ContextPack ambiguity/evidence/proposal/branch/trust signals rather than a fixed public extra-tag count. That slice itself did not persist diagnostics; doc 35 later adds correction-only near-miss snapshots.
54. The Conceptualize Extractor Flip decision is locked and implemented in doc 33. The first live flip is classification-only: the old extractor still owns card text extraction, while `conceptualizeClassification.ts` owns ContextPack -> prompt builder -> model call -> strict validator -> save-candidate adapter for ontology placement. `prepareSaveCandidates` accepts an optional `conceptualizeContext`; valid Conceptualize output replaces the ontology placement, invalid output falls back to the old extractor placement. `noStrongMatch` stays unclassified, and `suggestedNewConcept` is not mapped into `conceptHint.proposedConceptType`. Doc 37 now adds bounded user-fit history to the ContextPack prompt payload only. No diagnostic persistence, near-miss evidence snapshot, missing-concept UI, proposal creation, ontology/profile mutation, checker runtime, learned-score persistence, or auto-apply was added.
55. The raw proposed type identity decision is locked and implemented in doc 34. Save candidates now carry `rawProposedTypeIdentity` as the canonical in-memory shape: valid Conceptualize refs are scoped refs, and legacy extractor-invented invalid ids are unresolved raw ids with active scope context. `rawProposedTypeNodeId` remains a derived legacy string projection for existing correction evidence persistence. Structured identity wins if both fields are present. No DB migration, diagnostic persistence, missing-concept UI, proposal creation, ontology/profile mutation, checker runtime, user-fit projection, agent runtime, app-builder runtime, or DSL runtime was added.
56. The correction-evidence near-miss decision is locked and implemented in doc 35. Hidden Conceptualize diagnostic candidates now flow through save candidate memory as `conceptualizeNearMissCandidates` and persist only when a user correction writes `OntologyCorrectionEvidence`. Migration 020 adds `ontology_correction_evidence.near_miss_candidates_json`; the ontology codec/schema/backup support validate it as inert scoped candidate refs. Successful uncorrected saves do not persist diagnostics. Follow-up hardening rejects diagnostic candidates that reuse the public primary rank and tests that uncorrected saves cannot persist near-miss diagnostics. No visible extra tags, missing-concept UX, checker runtime, proposal creation, user-fit projection, confidence/ranking update, automatic ontology/profile mutation, DB-backed history readers, graph traversal, vector retrieval, old-card backfill, auto-apply, agent runtime, app-builder runtime, or DSL runtime was added.
57. The missing-concept UX decision is locked and implemented in doc 36. `SaveModalCandidateData` now carries `ConceptualizeMissingConceptReview` metadata for `noStrongMatch`; `conceptualizeClassification.ts` maps validated missing output into that review state while leaving `conceptHint`, `rawProposedTypeIdentity`, and `rawProposedTypeNodeId` null. The candidate card shows "Needs type review"; correction controls show "No strong existing type" and let the user explicitly copy `suggestedNewConcept` into manual `newTypeLabel` / `reason` fields. The store still initializes `newTypeLabel` to an empty string, so suggestions are not auto-filled or auto-applied. No automatic missing-concept apply, automatic proposal creation, ontology/profile mutation, checker runtime, DB-backed history readers, user-fit projection, confidence/ranking update, vector retrieval, graph traversal, old-card backfill, auto-apply, agent runtime, app-builder runtime, or DSL runtime was added.
58. The user-fit projection decision is locked and implemented in doc 37. `userFitProjection.ts` exposes `projectUserFitSignals(input)`, `profileChangeProposalTargetKey(target)`, and `userFitActiveSelectionScopeKey(snapshot)` as pure ontology helpers. The projection consumes caller-supplied `OntologyCorrectionEvidence[]` and `ProfileProposalEvent[]`, groups node and proposal signals, reports counts/evidence ids/event ids/latest timestamps, maps user-fit confidence to signed scores, and caps output with omitted counts. Node signals are scoped to the active profile selection where the correction happened, so plain `coding` and `coding + react-project` remain separate. Matching near-miss hits fold into that active correction scope instead of creating a second diagnostic-candidate scope. Missing-concept corrections and near-miss hits are weak positive evidence; postponed proposals are mild negative evidence; asked-why remains neutral. `userFitHistoryRepo.ts` exposes `loadUserFitProjectionFacts(input)` behind the ontology data boundary; it loads bounded recent correction/proposal facts for one base profile and returns facts only. Migration 021 adds composite recency indexes for those reads. `resolveConceptualizeProfileContext()` now projects those facts and Conceptualize includes matching current-scope node/proposal signals in the typed ContextPack/prompt payload as advisory history only. No learned-score persistence, trust setting update, proposal creation, checker runtime, auto-apply, vector retrieval, graph traversal, UI change, projection cache, or ontology/profile mutation was added.
59. The base profile versioning target contract is locked and implemented in doc 38. Migration 022 adds `profile_change_proposals.target_profile_version`, backup `SCHEMA_VERSION` is now 22, and `ProfileChangeProposal.targetProfileVersion` maps through schema, codec, repo, and backup boundaries. New base-targeted Conceptualize proposals snapshot `context.profile.version`; branch-targeted proposals reject non-null `targetProfileVersion` and keep using branch `updatedAt` guards. `baseProfileVersioning.ts` exposes pure `assertBaseProfileProposalTargetsCurrentVersion(input)` for base/core apply code. It rejects non-base proposals, base id mismatches, missing target versions, and stale versions before mutation. No base/core mutation, version-history table, stale refresh flow, UI behavior change, checker runtime, auto-apply, agent runtime, app-builder runtime, or DSL runtime was added in the target-contract slice.
60. The base profile apply helper/service is implemented under doc 38. `baseProfileProposalApply.ts` exposes pure `compileBaseProfileProposalApplyOperation`, `applyBaseProfilePatchOperation`, and `applyBaseProfileChangeProposal`. It accepts only pending non-merge base-target proposals, calls the target-version guard, revalidates patch conflicts against the current base profile, creates a typed `apply_profile_patch_to_base_profile` operation, and returns a next-version `ProfileDefinition` plus accepted proposal. The helper now documents that `patchToOverlay` is a synthetic one-shot composition adapter, not branch selection or persisted runtime-profile composition. It also mirrors branch-local apply by treating relationship type ids as opaque profile relationship ids while still rejecting duplicates/already-present ids, and tests `overrideOntology.nodes` as an add-only compatibility path. `ProfileChangeProposal` validation now rejects base targets where `target.profileId` diverges from `baseProfileId`. `data/baseProfileProposalApplyService.ts` exposes `applyPendingBaseProfileChangeProposal(input)` behind the ontology data barrel only. It loads the proposal and persisted profile definition in one transaction, conditionally writes the next profile definition version via `updateProfileDefinitionIfUnchanged`, conditionally accepts the pending proposal, and appends an `applied` proposal event with profile version details. No automatic base/core mutation, profile version-history table, stale proposal refresh flow, branch merge into base/core, auto-apply, checker runtime, old-card backfill, agent runtime, app-builder runtime, or DSL runtime was added in the helper/service slice.
61. The base/core proposal review UI wiring is implemented under doc 38. `useApplyProfileChangeProposal` routes `profile_branch` targets to branch-local apply and `base_profile` targets to `applyPendingBaseProfileChangeProposal`. `ProfileProposalReviewScreen` enables explicit base/core Apply, labels the action as `Apply to core`, and shows base/core warning/risk copy that derived branches are affected and old notes are not rewritten automatically. `profileProposalReviewPresentation.ts` maps base apply/versioning/write-conflict errors. Reject, Postpone, and Ask why stay shared. No auto-apply, stale refresh flow, edit-then-apply, branch merge apply, profile version-history UI, old-card backfill, checker runtime, historical undo, agent runtime, app-builder runtime, or DSL runtime was added.
62. The proposal freshness and stale refresh decision is locked and implemented in doc 40. Freshness is separate from proposal status: `fresh`, `unknown`, `stale_refreshable`, `conflicted`, and `obsolete` are derived review conditions, not persisted proposal statuses. Migration 024 adds `profile_change_proposals.target_branch_updated_at`, backup `SCHEMA_VERSION` is now 24, and `ProfileChangeProposal.targetBranchUpdatedAt` maps through schema, codec, repo, and backup boundaries. New branch-targeted Conceptualize proposals snapshot the selected target branch `updatedAt`. `profileProposalFreshness.ts` exposes pure `evaluateProfileProposalFreshness(input)`, which derives freshness from a proposal plus caller-supplied target facts and does not mutate, refresh, rebase, or apply anything. `profileProposalFreshnessService.ts` and `useProfileProposalFreshness()` load current target facts for the proposal review UI, classify freshness before Apply, and block Apply unless the derived state is `fresh`. Older branch proposals without the branch snapshot evaluate to `unknown`, not `fresh`. `profileChangeProposalRefreshService.ts` implements explicit refresh/rebase creation: when a pending proposal is stale-refreshable, it dry-runs the patch against the current target, inserts a replacement pending proposal with the current target-basis snapshot, supersedes the old proposal, and writes a `superseded` audit event in one transaction. `useRefreshProfileChangeProposal()` and `ProfileProposalReviewScreen` expose this as `Refresh proposal` only for refreshable proposals. Refresh does not mutate profiles, auto-apply, widen targets, rewrite cards/evidence, or create checker volume.
63. The doc 39 edited replacement service/hook is implemented. `profileChangeProposalEditService.ts` exposes `createEditedProfileChangeProposalReplacement(input)`: it loads a pending proposal in a transaction, preserves the original target/evidence, accepts an edited `ProfilePatch` plus edited copy/risk/confidence fields, snapshots the current target basis, dry-runs the patch through the branch-local or base-profile apply compiler, inserts a new pending user-authored replacement proposal, and reuses `supersedePendingProfileChangeProposal()` in the same transaction to mark the old proposal `superseded` and append the audit event. `useEditProfileChangeProposal()` adds user actor metadata/current time and invalidates proposal, event, and freshness queries. No target switching controls, direct Apply, automatic target widening, branch/base mutation during edit, checker runtime, auto-apply, old-card backfill, agent runtime, app-builder runtime, or DSL runtime was added.
64. The first visible doc 39 proposal editor UI is implemented in `ProfileProposalReviewScreen.tsx`. Supported pending new-node proposals expose an inline editor for label, parent id, meaning, reason, and risk. Saving builds an edited `ProfilePatch` through pure presentation helpers, calls `useEditProfileChangeProposal()`, creates a replacement pending proposal, supersedes the old proposal, and selects the replacement for normal review/apply. Unsupported patch shapes stay out of this first editor. Parent ids are validated in the shared branch/base proposal compile path, so edit-save, stale refresh, and final Apply reject parents that are not target item types. The target is not editable in this slice: no target-layer switching, silent widening, direct Apply from edit, branch/base mutation during edit, or old-card backfill was added.
65. The doc 41 manual checker runtime service is implemented behind the ontology data boundary, and the UI/model seam policy is locked. `data/checkerRunService.ts` exposes `runManualOntologyChecker(input)`: no active branch returns explanation-only with zero writes; active branch runs load bounded user-fit facts, resolve the base profile and target branch, assemble a checker `ContextPack`, build the checker prompt, call an injected checker model seam outside the write transaction, validate output, map supported findings to branch-local proposal candidates, dry-run each candidate through `compileBranchLocalProposalApplyOperation`, and insert only dry-run-valid proposals. The write transaction re-reads the target branch and current pending checker proposals, so created proposals use the fresh branch `updatedAt` snapshot and duplicates are skipped without rewriting existing proposals. Patch conflicts are skipped with read-only explanation. The service is exported only from `src/features/ontology/data/index.ts`; it is not exported from the root ontology barrel. No UI trigger, concrete LLM/provider adapter implementation, checker-run table, proposal event creation, auto-apply, base/core checker targeting, relationship/boundary operation, graph/vector retrieval, agent runtime, app-builder runtime, or DSL runtime was added.

Latest verification after Manual Checker Runtime Service:

```text
npx.cmd tsc --noEmit --pretty false
npm.cmd test -- --run src/features/ontology/__tests__/checkerPromptBuilder.test.ts src/features/ontology/__tests__/checkerProposalMapper.test.ts src/features/ontology/__tests__/checkerRunService.test.ts src/__tests__/stage10-architecture-guards.test.ts --reporter verbose
```

Result: TypeScript clean; focused checker prompt/mapper/runtime/stage10 guard tests 96/96 passed across 4 files.

Latest verification after Proposal Editor UI:

```text
node node_modules\typescript\bin\tsc -p tsconfig.json --noEmit
npm.cmd test -- --run src/features/ontology/__tests__/branchLocalProposalApply.test.ts src/features/ontology/__tests__/baseProfileProposalApply.test.ts src/features/ontology/__tests__/profileChangeProposalEditService.test.ts src/features/ontology/__tests__/profileProposalReviewPresentation.test.ts src/__tests__/stage10-architecture-guards.test.ts
npm.cmd test -- --run
```

Result: TypeScript clean; focused apply/editor/edit-service/guard tests 111/111 passed across 5 files; full suite 1005/1005 passed across 108 files.

Latest verification after Proposal Edit Replacement Service:

```text
node node_modules\typescript\bin\tsc -p tsconfig.json --noEmit
npm.cmd test -- --run src/features/ontology/__tests__/profileChangeProposalEditService.test.ts src/features/ontology/__tests__/useEditProfileChangeProposal.test.ts src/features/ontology/__tests__/profileChangeProposalLifecycleService.test.ts src/__tests__/stage10-architecture-guards.test.ts
```

Result: TypeScript clean; focused edit/lifecycle/hook/guard tests 92/92 passed across 4 files; full suite 1000/1000 passed across 108 files; `git diff --check -- ONTOLOGY_PROFILE_REFACTOR src ARCHITECTURE.md` clean with CRLF warnings only.

Latest verification after Proposal Freshness, Readout, And Refresh:

```text
node node_modules\typescript\bin\tsc -p tsconfig.json --noEmit
npm.cmd test -- --run src/features/ontology/__tests__/profileChangeProposalRefreshService.test.ts src/features/ontology/__tests__/useRefreshProfileChangeProposal.test.ts src/features/ontology/__tests__/profileProposalReviewPresentation.test.ts src/features/ontology/__tests__/profileProposalFreshness.test.ts src/features/ontology/__tests__/profileProposalFreshnessService.test.ts src/features/ontology/__tests__/useReviewProfileChangeProposal.test.ts
```

Result: TypeScript clean; focused refresh/hook/readout/freshness tests 28/28 passed across 6 files.

Latest verification after Base/Core Proposal Review UI wiring:
...
Latest verification after Base Profile Apply Helper/Service:
...
Latest verification after Base Profile Versioning:
...
Latest verification after User-Fit ContextPack wiring:

```text
node node_modules\typescript\bin\tsc -p tsconfig.json --noEmit
npm.cmd test -- --run src/__tests__/stage10-architecture-guards.test.ts src/features/ontology/__tests__/contextAssembly.test.ts src/features/ontology/__tests__/contextSelector.test.ts src/features/learning/services/__tests__/conceptualizeContextPack.test.ts src/features/learning/services/__tests__/conceptualizePromptBuilder.test.ts src/features/learning/services/__tests__/conceptualizeProfileContext.test.ts src/features/learning/services/__tests__/conceptualizeClassification.test.ts src/features/learning/services/__tests__/stage2-prepareSaveCandidates.test.ts
npm.cmd test -- --run
git diff --check -- ONTOLOGY_PROFILE_REFACTOR src ARCHITECTURE.md
```
...
Latest verification after Missing-Concept UX:
...
Prior verification after Correction Evidence Near-Miss:
...
Latest verification after Raw Proposed Type Identity:
...
Prior verification after Conceptualize Extractor Flip:
...
Prior verification after Conceptualize singular-output diagnostics:
...
Latest verification after proposal superseding, Ask-why audit wiring, and superseding hook boundary:
