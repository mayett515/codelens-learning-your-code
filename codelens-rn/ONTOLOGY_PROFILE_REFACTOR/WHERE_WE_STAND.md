...
This file captures the current state after implementing correction evidence persistence v1, locking the adaptive suggestion policy, implementing storage-only patch/merge proposal persistence v1, locking and implementing the first Conceptualize correction loop, adding raw-proposed-type preservation for normalized extractor mistakes, implementing storage-only trust settings, implementing the branch-local proposal apply helper/service, adding the first minimal proposal review UI, adding proposal event/audit storage for proposal decisions, implementing the first pure context assembly slice, implementing the first pure context selector slice, wiring the real Conceptualize flow to build/validate a ContextPack, implementing the pure Conceptualize prompt builder/output validator, hardening Conceptualize to use a singular public output plus internal dynamic diagnostic candidates, flipping Conceptualize classification live through a guarded classification-only adapter, making raw proposed type identity structured in memory while keeping the old string projection for current evidence persistence, persisting hidden near-miss diagnostics only when user correction evidence is written, surfacing missing Conceptualize matches as explicit review metadata, projecting correction/proposal facts into bounded user-fit signals, adding a DB-backed facts reader for bounded recent user-fit history, feeding matching current-scope user-fit signals into Conceptualize ContextPacks as advisory prompt context, adding a pure checker selector seam that can consume the same bounded `ContextPack.userFit` section, hardening manual missing-concept proposal creation with parent-id and node-id collision revalidation, adding the first base-profile proposal target-version guard, adding the first base-profile apply helper/service for persisted profile definitions, wiring explicit base/core apply into the proposal review UI, adding the first richer missing-concept draft slice with editable meaning plus proposal provenance, adding a Conceptualize-to-proposal-review handoff for created pending proposals, adding proposal superseding lifecycle persistence, exposing the superseding hook boundary, adding proposal freshness readout, adding explicit stale refresh replacement creation, locking the checker runtime first-slice doc as manual-on-demand, branch-local, proposal-only, additive ontology-node/item-type only, implementing the first pure checker prompt/output contract plus deterministic proposal mapper, implementing the manual checker runtime service behind the ontology data boundary, and locking the UI/model seam policy before UI/provider wiring.

## Last Status Response

Done.

Codex direct slice (Manual Checker Runtime Service):

- Implemented `src/features/ontology/data/checkerRunService.ts`.
- Exported `runManualOntologyChecker()` from the ontology data barrel only; the root ontology barrel does not export it.
- The service returns explanation-only with zero writes when no active branch is supplied.
- The service loads bounded user-fit facts, resolves the base profile and target branch, assembles a checker `ContextPack`, builds the checker prompt, calls an injected checker model seam outside the write transaction, validates output, maps supported findings to branch-local proposal candidates, dry-runs each candidate through `compileBranchLocalProposalApplyOperation`, and inserts only proposals that pass.
- The write transaction re-reads the target branch, uses the fresh branch `updatedAt` snapshot for `targetBranchUpdatedAt`, deduplicates against current pending checker proposals, and never updates/upserts/deletes existing proposals.
- Dry-run patch conflicts are skipped with read-only explanation instead of being persisted as conflicted queue clutter.
- Added focused runtime service tests plus mapper hardening tests for caller cap clamping and root-category/null-parent output.
- Added stage10 architecture guards proving the runtime stays data-boundary, branch-local, insert-only, manually triggered by caller, and free of auto-apply, trust-setting, scheduling, base/core, relationship/boundary, and proposal-event creation paths.
- No UI trigger, concrete LLM/provider adapter implementation, checker-run table, auto-apply, base/core checker targeting, relationship/boundary operation, graph/vector retrieval, agent runtime, app-builder runtime, or DSL runtime was added.
- Verification:
  - TypeScript clean
  - focused checker prompt/mapper/runtime/stage10 guard tests: 96/96 passed across 4 files

Codex direct slice (Checker Pure Prompt Contract And Mapper):

- Implemented `src/features/ontology/checkerPromptBuilder.ts`.
- Implemented `src/features/ontology/checkerProposalMapper.ts`.
- The checker prompt builder consumes only a validated checker `ContextPack`, renders a stable instruction shell plus bounded payload, and exposes `CheckerPromptOutputSchema`.
- The output validator accepts read-only explanation plus `missing_branch_item_type` findings and rejects unsupported operation kinds, unknown parent refs, unknown evidence ids, duplicate findings, non-checker packs, and model-supplied persistence metadata.
- The deterministic mapper converts validated findings into pending branch-local `ProfileChangeProposal` values with doc 41 pins: `sourceKind: 'checker'`, `sourceBranchId: null`, `proposalKind: 'ontology_node_patch'`, one concept/node per proposal, node `createdBy: 'model'`, node `status: 'active'`, branch target snapshot, deterministic low branch-local risk, cap-after-validation/dedup, and insert-only duplicate handling.
- No active branch produces explanation only and zero proposal values.
- Existing pending duplicates are skipped and reported in the read-only explanation; the mapper does not update, upsert, delete, or rewrite pending proposals.
- Added focused prompt/mapper tests and stage10 architecture guards.
- No DB fact loading, checker model seam, dry-run-before-insert, proposal insertion runtime, UI trigger, auto-apply, relationship/boundary operation, base/core checker targeting, mutation behavior, agent runtime, app-builder runtime, or DSL runtime was added.
- Verification:
  - TypeScript clean
  - focused checker prompt/mapper/stage10 guard tests: 88/88 passed across 3 files

Codex direct slice (Proposal Freshness, Readout, And Refresh):

- Added migration 024 for `profile_change_proposals.target_branch_updated_at`.
- Added `ProfileChangeProposal.targetBranchUpdatedAt` to the proposal codec/row mapping, repo writes, and backup column maps.
- Branch-targeted Conceptualize new-subtype proposals now snapshot the selected target branch `updatedAt`.
- Added pure `evaluateProfileProposalFreshness(input)` for `fresh`, `unknown`, `stale_refreshable`, `conflicted`, and `obsolete` derived review state.
- The freshness helper requires caller-supplied target facts and does not validate patches itself, mutate proposals, refresh/rebase, or apply anything.
- Added a read-only freshness service/hook that loads target facts, validates patch fit, and feeds the proposal review UI.
- Proposal review now shows freshness before Apply and blocks Apply unless the derived freshness state is `fresh`.
- Older branch proposals without `targetBranchUpdatedAt` remain valid and evaluate to `unknown`, not `fresh`.
- Added explicit refresh/rebase replacement creation for `stale_refreshable` proposals: refresh dry-runs the patch against the current target, creates a replacement pending proposal with the current target snapshot, supersedes the old pending proposal, and leaves the replacement for normal review.
- No checker runtime, auto-apply, old-card backfill, agent runtime, app-builder runtime, or DSL runtime was added.
- Verification:
  - TypeScript clean
  - focused refresh/hook/readout/freshness tests: 28/28 passed across 6 files
...
- No direct Apply from Conceptualize, target-layer switching UI, stale refresh/rebase, UI edit-flow editor that creates replacement proposals, old-card backfill, checker runtime, auto-apply, graph/vector retrieval, trust-setting update, agent runtime, app-builder runtime, or DSL runtime was added.
- Verification:
  - TypeScript clean
...
- Reject, Postpone, and Ask why stay shared; Ask why records audit history without changing proposal/profile state.
...
- Added the first pending proposal review surface without edit support:
  ...
  - Ask why / why not records audit history and does not mutate proposal/profile data
...
- Kept the original storage slice narrow: no user-fit projection, checker runtime, auto-apply engine, edit-then-apply, historical undo execution, base/core mutation, upward merge, old-card backfill, agent runtime, app-builder runtime, or DSL runtime.
- Verification so far:
  - TypeScript clean
...
- The branch-local proposal review/apply decision is locked in doc 24 and the helper/service plus first minimal UI slices are implemented: first apply is explicit, branch-local, revalidated, and atomic; first review actions are Apply, Reject, Postpone, and Ask why / why not; Ask why records audit history without changing proposal/profile state; risk/confidence wording explains blast radius; edit-then-apply, auto-apply, base/core mutation, upward merge, and old-card backfill stay future seams.
- The proposal event audit storage decision is locked and implemented in doc 25: Apply/Reject/Postpone/Ask-why/Superseded append `profile_proposal_events` inside guarded transactions or review action boundaries. Superseded and Ask-why are audit history, not user-fit preference. User-fit learning remains a future projection over those events.
...
Current update: doc 39 is locked and the first source slices are implemented. Conceptualize missing-concept drafts now carry editable meaning, `Use suggestion` copies suggested label/meaning/reason/valid parent, new subtype proposal nodes use edited meaning before suggestion/reason fallback, proposal reasons preserve original suggestion provenance plus edited-field provenance, Conceptualize correction controls show a read-only target/blast-radius summary, and saved drafts that create pending proposals can open the existing proposal review/apply surface with that proposal selected. Superseding lifecycle persistence is implemented: old pending proposals can be marked `superseded`, linked to an already-created pending replacement proposal, and recorded with a `superseded` proposal event. The edited replacement service/hook now validates a same-target edited patch, inserts a replacement pending proposal with the current target snapshot, and supersedes the old pending proposal in one transaction. The proposal review surface now has a first visible editor for supported new-node proposals; it edits label, parent id, meaning, reason, and risk, then saves through the same replacement/superseding hook. Doc 40 locks stale refresh as derived freshness plus replacement-pending-proposal superseding. The doc 40 freshness and refresh path is implemented: branch proposals can snapshot `targetBranchUpdatedAt`, `evaluateProfileProposalFreshness(input)` classifies freshness from caller-supplied target facts, proposal review shows freshness before Apply, blocks Apply unless the proposal is `fresh`, and `Refresh proposal` creates a replacement pending proposal for `stale_refreshable` proposals. Doc 41 now has the pure checker prompt/mapper and manual runtime service implemented behind the data boundary, plus the UI/model seam policy locked. No one-click direct Conceptualize Apply, target switching controls, checker UI trigger, concrete LLM/provider adapter implementation, auto-apply, or ontology/profile mutation was added.
...
The A2 decision for `prepareSaveCandidates` is locked and implemented. Docs 11-25 and 33-40 remain the current decision spine. Runtime coordination, correction evidence, branch persistence, profile selection, profile registry, runtime activation, profile definitions, adaptive suggestion policy, proposal storage, Conceptualize correction scope, trust settings, branch-local apply, base/core apply, proposal events, Extractor Flip, raw proposed type identity, near-miss snapshots, missing-concept UX, user-fit projection/history/ContextPack consumption, doc 39 draft/provenance/handoff/superseding/event-history/edit-replacement/editor slices, doc 40 branch revision/freshness/readout/refresh slices, and doc 41 pure checker/runtime slices are implemented. Doc 39 still lacks target switching and one-click direct Conceptualize Apply. Doc 41 still lacks the user-facing checker trigger/readout and concrete LLM/provider adapter implementation. The next work should be chosen against the locked docs, not re-decided from scratch.
...
The coordinator helper is now implemented and tested. The adaptive suggestion policy, proposal storage, Conceptualize preview/correction, checker/proposal/context/apply architecture, trust settings, branch-local apply, base/core apply, proposal events, ContextPack assembly/selection, prompt builder, Extractor Flip, raw proposed type identity, near-miss persistence, missing-concept UX/revalidation, user-fit projection/history/context consumption, doc 39 draft/provenance/handoff/superseding/event-history/edit-replacement/editor slices, and doc 40 branch revision/freshness/readout/refresh slices are implemented. The remaining open decision/implementation gaps require Codex plus human input:
