...
- The first second-base forkability proof is implemented: a minimal built-in `photography` base profile exercises registry, profile-definition codec/backup mapping, project selection, branch composition, manual checker proposals, branch-local apply, branch-to-base target switching, and photography base/core apply. The proof's collision finding is implemented: learning captures, concepts, save candidates, pre-check matching, concept-list filters, and retrieval filters now carry profile scope so overlapping node ids such as `composition` stay scoped by base profile.
