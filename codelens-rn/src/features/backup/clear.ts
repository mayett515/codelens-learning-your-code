...
export async function clearAllData(options?: ClearOptions): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.delete(schema.learningCaptures);
    ...
    await tx.delete(schema.ontologyCorrectionEvidence);
    await tx.delete(schema.profileProposalEvents);
    await tx.delete(schema.profileChangeProposals);
    await tx.delete(schema.profileTrustSettings);
